...
function createClientFromOperations(resources, defaultConfig) {
    ...
    const client = {
        request(url, input, config) {
            ...
            const baseUrl = function_1.pipe(O.tryCatch(() => new URL(url)), O.fold(() => mergedConf.baseUrl, url => url.origin));
            const parsedUrl = function_1.pipe(O.tryCatch(() => new URL(url)), O.getOrElseW(() => new URL(url, 'https://stoplight.io')));
            const httpUrl = {
                baseUrl,
                path: parsedUrl.pathname,
                query: querystring_1.parse(((_a = parsedUrl.search) === null || _a === void 0 ? void 0 : _a.substring(1)) || ''),
            };
