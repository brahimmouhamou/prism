...
const forward = ({ data: input, validations }, baseUrl, upstreamProxy, resource) => logger => function_1.pipe(NEA.fromArray(validations), TE.fromOption(function_1.constUndefined), TE.map(failedValidations => {
    ...
    const url = url_1.format(Object.assign(partialUrl, {
        pathname: path_1.posix.join(partialUrl.pathname || '', input.url.path),
        search: url_2.toURLSearchParams(input.url.query).toString(),
    }));
