...
function runCallback({ callback, request, response, }) {
    return logger => {
        ...
        return (0, function_1.pipe)(TE.tryCatch(() => (0, node_fetch_1.default)(url, requestData), E.toError), TE.chain(parseResponse_1.parseResponse), TE.mapLeft(error => logger.error({ name: 'CALLBACK' }, `${callback.callbackName}: Request failed: ${error.message}`)), TE.chainEitherK(element => {
