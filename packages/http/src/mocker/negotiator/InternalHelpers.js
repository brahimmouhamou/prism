...
const byResponseCode = (0, Ord_1.contramap)((response) => parseInt(response.code))(N.Ord);
function findLowest2xx(httpResponses) {
    const first2xxResponse = (0, function_1.pipe)(httpResponses, A.filter(response => /2\d\d/.test(response.code)), A.sort(byResponseCode), A.head);
    return (0, function_1.pipe)(first2xxResponse, O.alt(() => createResponseFromDefault(httpResponses, 200)));
}
exports.findLowest2xx = findLowest2xx;
function findFirstResponse(httpResponses) {
    return (0, function_1.pipe)(httpResponses, A.head);
}
exports.findFirstResponse = findFirstResponse;
function findResponseByStatusCode(responses, statusCode) {
    return (0, function_1.pipe)(responses, A.findFirst(response => response.code.toLowerCase() === String(statusCode)));
}
exports.findResponseByStatusCode = findResponseByStatusCode;
function createResponseFromDefault(responses, statusCode) {
    return (0, function_1.pipe)(responses, A.findFirst(response => response.code === 'default'), O.map(response => Object.assign({}, response, { code: statusCode })));
}
