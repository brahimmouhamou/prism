...
function runCallbacks({ resource, request, response, }) {
    return (0, withLogger_1.default)(logger => (0, function_1.pipe)(O.fromNullable(resource.callbacks), O.map(callbacks => (0, function_1.pipe)(callbacks, A.map(callback => (0, callbacks_1.runCallback)({ callback, request: parseBodyIfUrlEncoded(request, resource), response })(logger)())))));
}
...
function createInvalidInputResponse(failedValidations, responses, mockConfig) {
    ...
    return (0, function_1.pipe)((0, withLogger_1.default)(logger => logger.warn({ name: 'VALIDATOR' }, 'Request did not pass the validation rules')), R.chain(() => (0, function_1.pipe)(NegotiatorHelpers_1.default.negotiateOptionsForInvalidRequest(responses, expectedCodes, isExampleKeyFromExpectedCodes ? mockConfig.exampleKey : undefined), RE.mapLeft(error => {
